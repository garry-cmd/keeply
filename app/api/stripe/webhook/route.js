  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!userId) {
          console.error('checkout.session.completed: no userId');
          break;
        }
      ...
      case 'customer.subscription.updated': {
        ...
        if (!userId) {
          console.error('subscription.updated: cannot identify user', sub.customer);
          break;
        }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (!userId) userId = await getUserIdByCustomerId(sub.customer);
        if (!userId) {
          console.error('subscription.deleted: cannot identify user', sub.customer);
          break;
        }
      ...
      case 'invoice.payment_failed': {
        ...
        if (!userId) {
          console.log('Payment failed: cannot identify user', customerId);
          break;
        }
        ...
        if (!userEmail) {
          console.log('Payment failed: no email for userId', userId);
          break;
        }
        ...
        try {
          ...
          console.error('Portal URL error:', e);
        }
