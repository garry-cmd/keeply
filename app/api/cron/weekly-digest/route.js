...
export async function GET(request) {
  ...
  try {
    ...
    const eligibleUsers = users.filter(u => u.user_metadata && u.user_metadata.alertEmail === true);
    ...
    for (const user of eligibleUsers) {
      try {
        ...
        if (!members || members.length === 0) { skipped++; continue; }

        const vesselIds = members.map(m => m.vessel_id);
        const vesselMap = {};
        members.forEach(m => { if (m.vessels) vesselMap[m.vessel_id] = m.vessels; });
        ...
        if (totalAttention === 0 && repairsCount === 0) { skipped++; continue; }
...
function buildEmailHtml({ vesselMap, overdueTasks, upcomingTasks, openRepairs }) {
  const vesselNames = Object.values(vesselMap).map(v => v.vessel_name).join(", ");
