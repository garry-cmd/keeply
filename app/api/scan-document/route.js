...
export async function POST(request) {
  try {
    ...
    if (!supportedTypes.includes(mimeType)) {
      return Response.json({ error: "Unsupported file type. Please upload a JPG, PNG, or PDF." }, { status: 400 });
    }
...
Example: {"vessel_name":"IRENE","hin":"US-ABC12345D606","uscg_doc":"1234567","home_port":"Seattle, WA"}`
    ...
    try {
      ...
      return Response.json({ error: "Could not parse document. Please try a clearer photo." }, { status: 422 });
    }
