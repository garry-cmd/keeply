...
const ICP_REGIONS = [
  {
    ...
    isIcp: true,
    pattern:
      /seattle|bainbridge|port townsend|port ludlow|puget|bellingham|anacortes|tacoma|olympia|friday harbor|\bwa\b|washington(?! dc)|oregon|portland|astoria|\bor\b|british columbia|victoria/i,
  },
  {
    name: 'Florida / Gulf Coast',
    isIcp: true,
    pattern:
      /fort lauderdale|miami|tampa|jacksonville|pensacola|sarasota|clearwater|marathon|key west|\bfl\b|florida|gulf coast|new orleans|galveston|mobile|\bla\b|louisiana|\btx\b|texas/i,
  },
  {
    name: 'Northeast',
    isIcp: true,
    pattern:
      /new york|boston|newport|mystic|annapolis|chesapeake|baltimore|philadelphia|maine|connecticut|rhode island|\bmd\b|\bct\b|\bri\b|\bma\b|\bny\b|\bnj\b|\bme\b|new england|hamptons/i,
  },
  {
    name: 'Great Lakes',
    isIcp: true,
    pattern:
      /chicago|michigan|erie|ontario|superior|huron|green bay|cleveland|detroit|\bil\b|\bwi\b|\boh\b/i,
  },
];
function parseRegion(homePort: string): string {
  const hp = (homePort || '').toLowerCase();
  for (const r of ICP_REGIONS) {
    if (r.pattern.test(hp)) return r.name;
  }
...
export async function GET(req: NextRequest) {
  ...
  const annualPct =
    realActiveSubs.length > 0 ? Math.round((annualCount / realActiveSubs.length) * 100) : 0;
  ...
  const activationRate =
    allUsers.length > 0 ? Math.round((activatedUsers / allUsers.length) * 100) : 0;
  const neverActivated = allUsers.filter(
    (u) => u.email_confirmed_at && !vesselUserIds.includes(u.id)
  ).length;

  // WoW vessels
  const vesselsThisWeek = vesselData.filter((v) => new Date(v.created_at) > oneWeekAgo).length;
  const vesselsLastWeek = vesselData.filter(
  ...
  const regions = [...ICP_REGIONS.map((r) => r.name), 'Other / International'].map((name) => ({
    region: name,
  ...
  const day7Retention =
    cohort7.length > 0 ? Math.round((retained7.length / cohort7.length) * 100) : null;

  // ── First Mate ────────────────────────────────────────────────────────────────
  const firstMateThisMonth =
    firstMateResult.status === 'fulfilled'
      ? (firstMateResult.value.data ?? []).reduce(
  ...
  // ── Repairs WoW ──────────────────────────────────────────────────────────────
  const repairRows =
    repairCreatedResult.status === 'fulfilled' ? (repairCreatedResult.value.data ?? []) : [];
  const repairsThisWeek = repairRows.filter((r) => new Date(r.created_at) > oneWeekAgo).length;
  const repairsLastWeek = repairRows.filter(
  ...
  return NextResponse.json({
    ...
    product: {
      ...
      openRepairs:
        openRepairsResult.status === 'fulfilled' ? (openRepairsResult.value.count ?? 0) : 0,
      vesselsThisWeek,
      vesselsLastWeek,
      repairsThisWeek,
      repairsLastWeek,
