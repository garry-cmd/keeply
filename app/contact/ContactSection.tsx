...
const TYPES = [
  {
    ...
    placeholder: "What's going on? Include your vessel name or account email if it's account-related...",
  ...
  },
  {
    ...
    placeholder: "Tell us about your fleet — vessel count, marina/charter context, and what you're looking for...",
...
export default function ContactSection() {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '64px 24px 80px' }}>
        ...
        {TYPES.map(t => {
          ...
          return (
            <button
              ...
              style={{
                background: isActive ? `rgba(${t.value === 'support' ? '34,197,94' : t.value === 'fleet' ? '77,166,255' : '245,166,35'},0.08)` : 'rgba(7,30,61,0.6)',
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 5 }}>{t.label}</div>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', lineHeight: 1.55 }}>{t.description}</div>
              </div>
              <div style={{
                display: 'inline-flex',
                alignItems: 'center',
      ...
      >
        {status === 'sent' ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <div style={{ fontSize: 19, fontWeight: 700, color: '#fff', marginBottom: 8 }}>Message sent!</div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, maxWidth: 340, margin: '0 auto 24px' }}>
            ...
            <button
              ...
              style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.6)', padding: '9px 20px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Satoshi','DM Sans','Helvetica Neue',sans-serif" }}
            >
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
              <div>
                ...
                <input type="text" placeholder="Captain Garry" value={name} onChange={e => setName(e.target.value)} style={inp} />
              </div>
              <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={lbl}>Subject <span style={{ fontWeight: 400, opacity: 0.5 }}>(optional)</span></label>
              <input type="text" placeholder={active.subjectPlaceholder} value={subject} onChange={e => setSubject(e.target.value)} style={inp} />
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.28)' }}>
                Or email:{' '}
                <a href="mailto:support@keeply.boats" style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'underline', textDecorationStyle: 'dotted' }}>
              ...
              <button
                ...
                style={{
                  background: (!name.trim() || !email.trim() || !message.trim() || status === 'sending') ? 'rgba(255,255,255,0.08)' : active.color,
                  color: (!name.trim() || !email.trim() || !message.trim() || status === 'sending') ? 'rgba(255,255,255,0.3)' : '#07162d',
                  border: 'none',
                  borderRadius: 8,
                  padding: '11px 24px',
                  fontSize: 14,
                  fontWeight: 700,
                  cursor: (!name.trim() || !email.trim() || !message.trim() || status === 'sending') ? 'not-allowed' : 'pointer',
      ...
      {/* Response times */}
      <div style={{ marginTop: 20, background: 'rgba(7,30,61,0.5)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '18px 22px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 1 }}>
        ...
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7 }}>
          Support emails answered within 1 business day. Fleet inquiries within 24 hours. Feature requests reviewed weekly — we read them all and they directly influence the roadmap.
        </div>
      </div>
