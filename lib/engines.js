...
/**
 * Phase 2 helper (May 5, 2026) — return the hours-tracking *mode* of an
 * equipment row. Three values:
 *   'meter'         — equipment has its own hour meter
 *   'parent_engine' — inherits hours from a linked engine
 *   'none'          — date-based maintenance only
 *
 * Resolution order:
 *   1. Explicit hours_tracking column wins (set by AI onboarding or user
 *      via the Add/Edit Equipment radio).
 *   2. Legacy fallback for rows written before the column existed:
 *        - runtime_hours not null → 'meter' (someone entered a reading)
 *        - engine_id not null → 'parent_engine' (linked at insert time)
 *        - otherwise → 'none'
 *
 * Use this in UI to decide whether to render the runtime-hours strip and
 * which variant. Do NOT use it in the urgency calculator — that path goes
 * through getEngineHoursForTask which already handles the value resolution.
 */
export function getEquipmentHoursMode(eq) {
  if (!eq) return 'none';
  if (eq.hours_tracking === 'meter') return 'meter';
  if (eq.hours_tracking === 'parent_engine') return 'parent_engine';
  if (eq.hours_tracking === 'none') return 'none';
  // Legacy rows with no explicit column value: infer from data shape.
  if (eq.runtime_hours != null) return 'meter';
  if (eq.engine_id) return 'parent_engine';
  return 'none';
}
