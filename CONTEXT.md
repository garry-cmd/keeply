...
**Updated:** April 27, 2026 (late PM, post-marketing-pass + PostHog walkthrough + perf diagnosis)  
**Phase:** GoLive Ready · Marketing site cleaned + screenshot pass · PostHog confirmed live · Mobile perf is the now-known funnel leak  
...
- Push notifications
- **Lists tab (Apr 26, founder-only beta)** — Need to buy (procurement queue via `saved_parts`, sectioned R&M vs General) · Supplies (inventory via `supplies` + managed `vessel_locations`, auto-pulls to Need to buy on out-of-stock or below-threshold) · Haulout (queue via `requires_haul_out` flag on tasks/repairs). Bookmark icon on all 9 AI parts result surfaces. Mark received → Supplies handoff. **First Mate is Lists-aware** — answers "what do I need to buy", "do I have an X", "what's queued for haulout" using live data; haulout-flagged items excluded from active maintenance/repairs. All users currently see the LandHoShell coming-soon view; flip via `user_profiles.beta_features` text[] array.
...
- **Apr 26 late PM additions:**
  - **Messaging audit (HIGH).** 5 root-metadata strings in `app/layout.tsx` still carry old "AI-powered" / "Your vessel's First Mate, always ready" framing (root description, OG title, OG description, OG image alt, Twitter title, Twitter description). About page + hero now coverage-led. Picking the canonical Keeply one-liner is a real strategic call — needs fresh head, not 11pm. Once chosen, propagate to root metadata, App Store listing copy, Google Ads, and email templates as a single sweep. Filed in BACKLOG.
  ...
  - **3-phone Hero #2 follow-up:** when SV IRENE walkthrough video gets recorded, swap each `PhoneScreenshot` for a `<video>` element. Pattern is one-line per phone.
...
5. **Always end coding sessions with PowerShell commands.** Single fenced block, copy-paste order, no inline checklists/confirm prompts. Default ends with `git push origin HEAD:main`. For staging-first changes, end first block with staging push, brief "test, then run:" note, then a separate block for the main push.
6. **Code and docs deploy together.** Any PowerShell deploy bundle that ships code must either include matching CONTEXT.md / OKR page updates in the same commit, or Claude must explicitly state "no context changes needed" at the top of the deploy instructions. Caveat (Apr 26): for sessions with many incremental commits, batch the docs update once at end of session rather than touching CONTEXT.md mid-session — reduces noise and avoids regenerating patches as changes evolve. Never ship code in one push and reconcile docs in the next session — that is exactly how "Database-driven pricing" made it into CONTEXT.md for months while the code did something else entirely.
7. **History note (Apr 26):** Old policy was "always staging then prod, never main directly." Reversed because 14+ same-day deploys never tested the staging URL between pushes (it was paying ceremony) AND a Features CTA fix was lost when the second push step never ran. New default reflects what was actually safe in practice.
...
- ~~Never reposition as "maintenance app" — vessel intelligence~~ — **CONTESTED as of Apr 26.** About page scrub demoted "vessel intelligence platform" → "where your boat's whole record lives" and demoted First Mate from "primary interaction layer" to "help when you want it." Hero copy now leads with coverage ("every system, every part, every passage"), not AI. Root metadata + OG cards + Twitter still carry the old "Your vessel's First Mate, always ready" framing. **Action item:** "Messaging audit — pick canonical Keeply one-liner" — see new BACKLOG entry. Until that audit happens, do NOT silently propagate either framing; use coverage-led copy on website surfaces, leave metadata/social as-is so the conflict stays visible.
