...
**Updated:** April 26, 2026 (PM)  
**Phase:** GoLive Ready · Lists feature in founder-only beta  
...
- Push notifications
- **Lists tab (Apr 26, founder-only beta)** — Need to buy (procurement queue via `saved_parts`) · Supplies (inventory via `supplies` + managed `vessel_locations`) · Haulout (queue via `requires_haul_out` flag on tasks/repairs). Bookmark icon on all 9 AI parts result surfaces. Mark received → Supplies handoff. All users currently see the LandHoShell coming-soon view; flip via `user_profiles.beta_features` text[] array.
