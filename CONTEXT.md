...
- **Apr 26 session — Lists Session 1 shipped.** New "Lists" tab added to bottom nav (in the slot freed by removing the redundant First Mate tab — FM is reachable via the persistent top bar that was already in place). All users see the "Land ho!" coming-soon shell with three muted preview pills (Supplies / Grocery / Haulout), a lighthouse hero, three preview cards, and a "Notify me" CTA writing to a new `lists_waitlist` table. Shell-only today: real Supplies/Grocery/Haulout views ship in subsequent sessions behind a `user_profiles.beta_features` text[] gate. Strategic trigger: founder dogfooding revealed Keeply was additive (Garry still kept 4 Todoist lists after a month). Beta testers will hit the same friction; closing it is a credibility move pre-store-submission. Full plan: `lists-build-plan.md` in project knowledge.
