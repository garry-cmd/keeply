...
- **Apr 26 session — Lists Session 1 + UX cleanup pass shipped.** New "Lists" tab added to bottom nav (in the slot freed by removing the redundant First Mate tab — FM is reachable via the persistent top bar that was already in place). All users see the "Land ho!" coming-soon shell with three muted preview pills (Supplies / Grocery / Haulout), a lighthouse hero, three preview cards, and a "Notify me" CTA writing to a new `lists_waitlist` table. Shell-only today: real Supplies/Grocery/Haulout views ship in subsequent sessions behind a `user_profiles.beta_features` text[] gate. Strategic trigger: founder dogfooding revealed Keeply was additive (Garry still kept 4 Todoist lists after a month). Beta testers will hit the same friction; closing it is a credibility move pre-store-submission. Full plan: `lists-build-plan.md` in project knowledge. **Same-day cleanup pass (separate commit on top of Session 1):** (a) prefab quick-question chips below the FM bar — 4 horizontal-scrolling chips (What's overdue? / Parts I need / Ready to sail? / Before haul-out?) wired to the existing `fmPending` mechanism, restoring the popular suggestion-chips UX that lived only inside the now-unreachable FirstMateScreen; (b) vessel-info card pill row (ID / Docs / Admin / Ref / Edit) hidden by default — only renders when `isExpanded`, freeing vertical space on the My Boat tab. Tap the vessel header to reveal. **Open issue carried forward:** users cannot attach photos to equipment cards. Code path looks structurally correct (storage bucket + RLS + JSONB column all check out); investigation deferred pending an actual error message from devtools.
