...
(function patchPricing() {
  ...
  src = src.replace(/(\bbasic\b[\s\S]{0,300}?equipment[_]?[Cc]ards?\s*[:=]\s*)3/i, '$11');
  src = src.replace(/(\bfree\b[\s\S]{0,300}?equipment[_]?[Cc]ards?\s*[:=]\s*)3/i, '$11');
  // Generic fallback: maxEquipment: 3 → 1 near 'basic' or 'free'
  src = src.replace(/(maxEquipment\s*:\s*)3/g, '$11');
  src = src.replace(/(equipmentLimit\s*:\s*)3/g, '$11');
  ...
  if (!/pro[\s\S]{0,300}trialDays/i.test(src)) {
    src = src.replace(/(\bpro\b[\s\S]{0,20}?\{)/i, '$1\n    trialDays: 14,');
  }
...
(function patchLanding() {
  ...
  src = src.replace(/3\s*[Ee]quipment\s*[Cc]ards?/g, '1 Equipment Card (Engine)');
  src = src.replace(/Up to 3 equipment/gi, 'Up to 1 equipment');
  src = src.replace(/3 equipment items/gi, '1 equipment card (Engine)');

  // Pro trial: find Pro plan display and add trial badge
  // Look for Pro price display and add "14-day free trial" nearby
  src = src.replace(/(Pro[\s\S]{0,100}?\$25[\s\S]{0,200}?)(\/mo|per month)/i, '$1$2');
