...
16. **Stripe orphan subscriptions on user delete + duplicate customer on resignup.** Today, deleting a Supabase user via `DELETE FROM auth.users` does not cancel their Stripe subscription. The sub keeps billing the card; the renewal webhook then fails silently because the user_profiles row is gone. If the same email later resigns up, the checkout flow falls through to `customer_email` (Stripe doesn't dedupe by email) and creates a second Stripe customer alongside the orphan — potentially double-billing the cardholder. The Apr 21 fix only addressed stale `stripe_customer_id` pointing to a deleted Stripe customer; this is the inverse failure (Stripe live, Supabase gone). Three layered fixes in increasing cost: **(a)** add "cancel Stripe sub before deleting user" to the manual-deletion runbook (zero code, ~30 sec per delete); **(b)** add a `GET /v1/customers/search?query=email:'X' AND deleted:'false'` call at the top of `app/api/stripe/checkout/route.js` before the `customer_email` fallback, attach to the matched customer if found (~30 min, eliminates duplicate-customer for legit resignups); **(c)** soft-delete users with a `deleted_at` column instead of hard-delete, RLS hides them but Stripe link survives (~2 hr, architecturally correct, not a launch blocker). Today's mitigation is (a). Test mode = no real money so the issue is hygiene only there; the risk is real-mode users only.
17. **In-app alerts (bell icon in header) — build the feature the toggle promised.** On Apr 26 the "In-app alerts" row was removed from Settings → Alert Notifications because the underlying bell icon never shipped — the toggle saved to `user_metadata.alertInApp` but nothing read it, and there was no bell rendered anywhere. The `alertInApp` field is still loaded and saved (defaults to `true`) so existing user prefs survive. To restore the row: (a) add a bell icon to the header next to the vessel switcher, (b) compute a badge count from the same data the daily push cron uses (overdue tasks + due-soon tasks + open repairs from `loadFleetData()` shape, scoped to active vessel), (c) clicking the bell opens a slide-out panel listing the items grouped by urgency with deep-links to the task/repair, (d) re-add the toggle in Settings (gates whether the badge shows; bell icon itself can stay visible always). Effort estimate: ~3-4 hours. Strategic value: completes the "AI-first vessel intelligence" framing — the Active Cruiser persona expects to glance at the boat and see immediately what needs attention. Push + email give async reminders; the bell gives the in-session "what's pending right now" signal.
18. **Granular alert timing controls — wire or remove.** The Settings save handler writes `alertOverdue`, `alert3day`, `alert7day`, `alertDayOf`, `digestTime`, and `timezone` to user metadata. Toggles for these likely exist below the cut in the Settings panel (not yet audited). The weekly digest cron at `/api/cron/weekly-digest` ignores all of them — it always sends the same Monday-morning briefing. Two options: **(a)** delete the unused toggles from the UI (~15 min, honest), **(b)** wire the cron to respect them — per-user `digestTime`/`timezone` for send hour, `alert3day`/`alert7day`/`alertDayOf` to vary which task urgency tiers trigger an email outside the weekly cadence (~2-3 hrs, real product polish). Pick (a) before a tester notices, then revisit (b) when there's signal users want it. Skipping until post-launch is fine — these are deeper in the panel and unlikely to be the first thing a tester touches.
