...
const s = {
  ...
  btn: (primary) => ({
    fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 7, border: "none",
...
export default function PartsPage({ equipment, onBack, cart, onAddToCart }) {
  ...
  const categories = useMemo(function () {
    const cats = [...new Set((equipment || []).map(function (e) { return e.category; }))].sort();
    return ["All", ...cats];
  }, [equipment]);
  ...
  const grouped = useMemo(function () {
    ...
    visible.forEach(function (p) {
      if (!map[p._eqId]) map[p._eqId] = { eqName: p._eqName, eqCategory: p._eqCategory, parts: [] };
  ...
  const SECTIONS = {
    ...
    Navigation: "🧭", Sails: "⛵", Hull: "⚓", Deck: "🪝",
    Electronics: "📡", Vessel: "⚓", Other: "🔧",
  };
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", paddingBottom: 80 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 1 }}>
              {allParts.length} saved {allParts.length === 1 ? "part" : "parts"} across {(equipment || []).filter(function (e) { return (e.customParts || []).length > 0; }).length} equipment items
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ position: "relative", marginBottom: 10 }}>
        ...
        <input
          ...
          style={{ width: "100%", boxSizing: "border-box", border: "0.5px solid var(--border)", borderRadius: 10, padding: "9px 12px 9px 34px", fontSize: 13, background: "var(--bg-card)", color: "var(--text-primary)", fontFamily: "inherit", outline: "none" }}
        />
        {search && (
          <button onClick={function () { setSearch(""); }} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", fontSize: 16 }}>✕</button>
        )}
      </div>
          ...
          {categories.map(function (cat) {
            ...
            return (
              <button key={cat} onClick={function () { setFilter(cat); }}
                style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid " + (active ? "var(--brand)" : "var(--border)"), background: active ? "var(--brand-deep)" : "var(--bg-subtle)", color: active ? "var(--brand)" : "var(--text-muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                {active && "✓ "}{cat}
              </button>
            );
          })}
      ...
      {allParts.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--text-muted)" }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text-secondary)", marginBottom: 6 }}>No parts saved yet</div>
          <div style={{ fontSize: 13, lineHeight: 1.5 }}>
            Open any equipment card → Parts tab → AI suggestions → 💾 Save,<br />
        ...
        return (
          <div key={group.eqName} style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-secondary)" }}>{group.eqName}</span>
              <div style={{ flex: 1, height: "0.5px", background: "var(--border)" }} />
              <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "DM Mono, monospace" }}>{group.parts.length}</span>
            </div>
            ...
            <div style={{ background: "var(--bg-card)", border: "0.5px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              {group.parts.map(function (part, idx) {
                ...
                const searchUrl = hasPartNum
                  ? "https://www.google.com/search?q=" + encodeURIComponent(part.sku + " " + part.name + " marine")
                  : "https://www.google.com/search?q=" + encodeURIComponent(part.name + " " + group.eqName + " marine");

                return (
                  <div key={part.id} style={{
                    padding: "12px 14px",
                    borderBottom: idx < group.parts.length - 1 ? "0.5px solid var(--border)" : "none",
                    display: "flex", alignItems: "flex-start", gap: 10,
                  }}>
                    {/* Left: name + metadata */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: hasPartNum || part.notes ? 4 : 0 }}>
                        {part.name}
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                      {/* Buy / Search link */}
                      <a href={hasUrl ? part.url : searchUrl}
                        target="_blank" rel="noreferrer"
                        style={{ ...s.btn(false), textDecoration: "none", display: "inline-block", lineHeight: "22px" }}>
                        {hasUrl ? "↗ Buy" : "🔍"}
                      </a>
