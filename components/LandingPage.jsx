...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "'Satoshi','DM Sans','Helvetica Neue',sans-serif", color: WHITE, background: NAVY, overflowX: "hidden" }}>
      ...
      {/* Auth Modal */}
      {showAuth && (function(){
        var authInput = { width: "100%", padding: "11px 13px",
                          border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10,
                          fontSize: 14, outline: "none", boxSizing: "border-box",
                          background: "rgba(255,255,255,0.05)", color: "#fff",
                          fontFamily: "inherit" };
        var authLabel = { fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.7)",
                          display: "block", marginBottom: 6, letterSpacing: "0.2px" };
        var primaryBtn = function(disabled){ return { width: "100%", padding: "12px 0",
                          background: disabled ? "rgba(245,166,35,0.4)" : GOLD, color: "#1a1200",
          ...
          <div style={{ background: NAVY, borderRadius: 16, padding: "28px 28px",
            ...
            {signupEmail ? (
              ...
              <div style={{ textAlign: "center", padding: "12px 0 4px" }}>
                <div style={{ width: 56, height: 56, margin: "0 auto 16px", borderRadius: 14,
                              background: stripeSuccess ? "rgba(74,222,128,0.15)" : "rgba(245,166,35,0.15)",
                              display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
                       stroke={stripeSuccess ? "#4ade80" : GOLD} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    {stripeSuccess
                      ? <polyline points="20 6 9 17 4 12" />
                      : <g><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></g>
                    }
                  </svg>
                </div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", marginBottom: 10, letterSpacing: "-0.3px" }}>
                  {stripeSuccess ? "Payment confirmed" : "Check your inbox"}
                </div>
                <div style={{ fontSize: 14, color: "rgba(255,255,255,0.65)", lineHeight: 1.6, maxWidth: 340, margin: "0 auto" }}>
                  {stripeSuccess
                    ? "Your subscription is active. We've sent a confirmation link to your email — click it to come aboard."
                    : <span>We sent a confirmation link to <strong style={{ color: "#fff", fontWeight: 600 }}>{signupEmail}</strong>. Click it to activate your account and come aboard.</span>
                  }
                </div>
              </div>
            ) : isRecovery ? (
              ...
              <>
                <div style={{ textAlign: "center", marginBottom: 20, paddingTop: 4 }}>
                  <div style={{ width: 48, height: 48, margin: "0 auto 14px", borderRadius: 12,
                                background: "rgba(77,166,255,0.12)",
                                display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
                         stroke={ACCENT} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                      <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                      <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                    </svg>
                  </div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", marginBottom: 6, letterSpacing: "-0.3px" }}>
                    Set a new password
                  </div>
                ...
                <form onSubmit={updatePassword}>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <input type={showPwd ? "text" : "password"} value={confirmPassword}
                      ...
                      placeholder="At least 6 characters" required minLength={6}
                      style={{ ...authInput,
                        borderColor: confirmPassword && confirmPassword !== password ? "#f87171" : "rgba(255,255,255,0.15)" }} />
                    {confirmPassword && confirmPassword !== password && (
                      <div style={{ fontSize: 12, color: "#f87171", marginTop: 6 }}>Passwords don{"'"}t match</div>
                    )}
                  </div>
                  {error   && <div style={{ fontSize: 13, color: "#f87171", marginBottom: 12, lineHeight: 1.5 }}>{error}</div>}
                  {message && <div style={{ fontSize: 13, color: "#4ade80", marginBottom: 12, lineHeight: 1.5 }}>{message}</div>}
                  <button type="submit" disabled={loading || (confirmPassword !== "" && confirmPassword !== password)}
                    style={primaryBtn(loading || (confirmPassword !== "" && confirmPassword !== password))}>
              ...
              <>
                {/* Brand header */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 14, paddingTop: 2 }}>
                  <svg width="22" height="22" viewBox="0 0 36 36" fill="none" aria-hidden="true">
                    <path d="M18 2L4 7.5V18c0 7.5 6 13.5 14 16 8-2.5 14-8.5 14-16V7.5L18 2Z" fill={BRAND}/>
                    <circle cx="18" cy="18" r="7.2" stroke="white" strokeWidth="2" fill="none"/>
                    <path d="M13.5 18l3.2 3.2L23 13.5" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span style={{ fontSize: 16, fontWeight: 700, color: "#fff", letterSpacing: "-0.2px" }}>Keeply</span>
                </div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", textAlign: "center", marginBottom: 4, letterSpacing: "-0.3px" }}>
                  {mode === "signup" ? "Create your account" : "Welcome back"}
                </div>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", textAlign: "center", marginBottom: 18 }}>
                  {mode === "signup" ? "No credit card required" : "Log in to your account"}
                </div>

                {/* Plan context strip (signup only, excludes free) */}
                {mode === "signup" && pendingPlan && pendingPlan !== "free" && PRICING_CONFIG[pendingPlan] && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10,
                                background: "rgba(77,166,255,0.08)",
                                border: "1px solid rgba(77,166,255,0.25)",
                                borderRadius: 10, padding: "9px 12px", marginBottom: 16 }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: ACCENT, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 1 }}>
                        Signing up for
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>
                        {pendingPlan === "standard" ? "Standard" : pendingPlan === "pro" ? "Pro" : pendingPlan}
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.45)", textDecoration: "line-through", lineHeight: 1.2 }}>
                ...
                <button onClick={signInWithGoogle} disabled={loading}
                  ...
                  onMouseEnter={function(e){ if(!loading) e.currentTarget.style.background = "rgba(255,255,255,0.1)"; }}
                  onMouseLeave={function(e){ if(!loading) e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                  ...
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.8px" }}>or</span>
                  <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} />
                </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                      ...
                      {mode === "login" && (
                        <button type="button" onClick={resetPassword} disabled={loading}
                          style={{ fontSize: 12, color: ACCENT, background: "none", border: "none",
                                   cursor: "pointer", padding: 0, fontWeight: 500, fontFamily: "inherit" }}>
                          Forgot password?
                        </button>
                      )}
                    </div>
                    <div style={{ position: "relative" }}>
                      ...
                      <button type="button" onClick={function(){ setShowPwd(function(v){ return !v; }); }}
                        ...
                        style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)",
                                 ...
                                 color: "rgba(255,255,255,0.5)", padding: 6,
                                 display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                          {showPwd
                            ? <g><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></g>
                ...
                <div style={{ textAlign: "center", marginTop: 16, fontSize: 13, color: "rgba(255,255,255,0.5)" }}>
                  ...
                  <button type="button"
                    onClick={function(){ setMode(mode === "signup" ? "login" : "signup"); setError(null); setMessage(null); }}
