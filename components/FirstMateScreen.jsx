...
async function fetchVesselContext(vesselId) {
  ...
  const [vR, tR, rR, lR, eR] = await Promise.all([
    ...
    fetch(`${SUPA_URL}/rest/v1/repairs?vessel_id=eq.${vesselId}&order=date.desc&select=*`, {
      headers: h,
    }),
    fetch(
      `${SUPA_URL}/rest/v1/logbook?vessel_id=eq.${vesselId}&order=entry_date.desc&limit=20&select=*`,
      { headers: h }
    ),
    fetch(
  ...
  return {
    ...
    tasks: (tasks || []).map(function (t) {
      const recentLogs = (t.service_logs || [])
        .slice(-5)
        .reverse()
        .filter(function (l) {
          return l.comment;
    ...
    equipment: (equipment || []).map(function (e) {
      var rawLogs = Array.isArray(e.logs)
        ? e.logs
        : typeof e.logs === 'string'
          ? JSON.parse(e.logs || '[]')
          : [];
      var recentLogs = rawLogs
        .slice(-10)
        .reverse()
        .map(function (l) {
...
export default function FirstMateScreen({
  vesselId,
  vesselName,
  vesselType,
  tasks,
    ...
    const urgent = tasks
      ...
      .map(function (t) {
        ...
        return {
          title: t.task,
          body:
            (daysAgo !== null ? 'Last done ' + daysAgo + ' days ago.' : 'Never serviced.') +
            (t.recentNotes && t.recentNotes.length > 0
              ? ' Note: "' + t.recentNotes[0].split(': ').slice(1).join(': ') + '"'
              : ''),
          urgent: u === 'critical',
          section: t.section,
        };
  ...
  const send = useCallback(
    async function (text) {
      ...
      setMessages(function (prev) {
        return [...prev, { role: 'assistant', content: '', loading: true, id: thinkId }];
      });
      try {
        const sess = await supabase.auth.getSession();
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Avatar size={36} />
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: D.text, lineHeight: 1.2 }}>
              First Mate
            </div>
        ...
        {isLimited && (
          <div
            style={{
              ...
              border:
                '1px solid ' + (isAtLimit ? 'rgba(239,68,68,0.35)' : 'rgba(255,255,255,0.12)'),
            }}
          >
      ...
      >
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border:
                      '0.5px solid ' +
                      (ins.urgent ? 'rgba(239,68,68,0.3)' : 'rgba(245,166,35,0.2)'),
                    borderRadius: 10,
                    padding: '10px 12px',
                    marginBottom: 8,
                  }}
                >
                  <div
                    style={{
                      fontSize: 12,
                      fontWeight: 700,
                      color: ins.urgent ? '#fca5a5' : D.text,
                      marginBottom: 3,
                    }}
                  >
                    {ins.title}
                  </div>
                  <div style={{ fontSize: 11, color: D.muted, lineHeight: 1.45, marginBottom: 8 }}>
                    {ins.body}
                  </div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button
                      onClick={function () {
                        send('How urgent is the ' + ins.title + ' and what do I need to fix it?');
                      }}
                      style={{
                        fontSize: 10,
                        fontWeight: 700,
                    ...
                    <button
                      onClick={function () {
                        send('What parts do I need for ' + ins.title + '?');
                      }}
                      style={{
                        fontSize: 10,
                        fontWeight: 700,
        ...
        <div style={{ padding: insights.length > 0 ? '8px 14px 14px' : '14px 14px' }}>
          <button
            onClick={function () {
              send(
                'Is ' +
                  vName +
                  ' ready for a passage? Give me a complete departure readiness briefing — overdue items, open repairs, equipment concerns, and anything I should address before casting off.'
              );
            }}
            style={{
              width: '100%',
          ...
          >
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#1a1200' }}>
                Is {vName} ready to go?
              </div>
        ...
        {messages.length === 0 ? (
          <div
            style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0 14px 14px' }}
          >
            <div
              style={{ height: '0.5px', background: 'rgba(255,255,255,0.07)', marginBottom: 14 }}
            />
            <div
              style={{
                fontSize: 10,
          ...
          >
            <div
              style={{ height: '0.5px', background: 'rgba(255,255,255,0.07)', marginBottom: 2 }}
            />
          ...
          >
            ...
            >
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                stroke={micActive ? '#f87171' : '#f5a623'}
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
            ...
            <button
              ...
              style={{
                ...
                flexShrink: 0,
                background:
                  input.trim() && !loading && context
                    ? 'linear-gradient(135deg,#0f4c8a,#4da6ff)'
