...
function StripeCheckout({ cart, onSuccess, onClose }) {
  ...
  const inp = (field, ph, half) => <input placeholder={ph} value={form[field]} onChange={e => setForm({ ...form, [field]: e.target.value })} style={{ width: half ? "calc(50% - 6px)" : "100%", border: "1.5px solid #e2e8f0", borderRadius: 8, padding: "10px 12px", fontSize: 14, background: "#fafafa", boxSizing: "border-box", outline: "none" }} />;
  return (
...
export default function App() {
  ...
  const toggleTask = async (id) => {
    ...
    setTasks(prev => prev.map(t => t.id !== id ? t : {
      ...
      due_date:     newDue,      dueDate:     newDue,
  ...
  const saveVessel = async () => {
    ...
    const payload = {
      vessel_name: settingsForm.vesselName,
      vessel_type: settingsForm.vesselType || 'sail',
      owner_name:  settingsForm.ownerName  || '',
      home_port:   settingsForm.address    || '',
      make:        settingsForm.make       || '',
      model:       settingsForm.model      || '',
      year:        settingsForm.year       || '',
    };
    if (editingVesselId) {
      const { data, error } = await supabase
        .from('vessels').update(payload)
        .eq('id', editingVesselId).select().single();
      ...
      setVessels(vs => vs.map(v => v.id === editingVesselId ? mapped : v));
    } else {
      const { data, error } = await supabase
        .from('vessels').insert(payload).select().single();
      ...
      const seedTasks = MAINTENANCE_TASKS.map(t => ({
        ...
        task:          t.task,
        section:       t.section,
        interval_days: intervalToDays(t.interval),
        priority:      t.priority,
        last_service:  t.lastService,
        due_date:      addDays(t.lastService, intervalToDays(t.interval)),
        service_logs:  [],
      }));
      const { error: seedError } = await supabase.from('maintenance_tasks').insert(seedTasks);
  ...
  if (loading) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: 'sans-serif', color: '#6b7280', fontSize: 14 }}>Loading…</div>;
  }
  ...
  return (
    <div style={s.app} onClick={() => setShowVesselDropdown(false)}>
      ...
      {orderSuccess && <div style={{ background: "#16a34a", color: "#fff", padding: "12px 24px", fontSize: 14, fontWeight: 600, textAlign: "center" }}>✅ Order {orderSuccess} confirmed!</div>}
...
{eq.other && <span style={{ fontStyle: "italic", color: "#b0b8c8" }}> · {eq.other}</span>}
                            ...
                            {activeTab === "parts" && (<>
                                  ...
                                  <div key={part.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: idx < allParts.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 12, flexShrink: 0 }}>
                                      ...
                                      {part.vendor !== "custom" && (inCart
                                        ? <div style={{ display: "flex", gap: 5, alignItems: "center" }}><span style={{ fontSize: 11, color: "#16a34a", fontWeight: 700 }}>✓</span><button onClick={() => removeFromCart(part.id)} style={{ background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}>✕</button></div>
                                        : <button onClick={() => addToCart(part)} style={{ background: "#0f4c8a", color: "#fff", border: "none", borderRadius: 8, padding: "5px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>+ Add</button>)}
                                    </div>
                                  </div>
                                );
                              })}
                              ...
                              {allParts.some(p => cart.find(i => i.id === p.id)) && (
                                <div style={{ marginTop: 10, display: "flex", justifyContent: "flex-end" }}>
                                  <button onClick={() => setShowCartPanel(true)} style={{ background: "#f0fdf4", color: "#16a34a", border: "1px solid #86efac", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>View Cart ({cartQty}) →</button>
                                </div>
                              )}
                            </>)}
                ...
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
                    ...
                    <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "12px 14px" }}>
                      <div style={{ fontSize: 22, fontWeight: 800, color: "#0f4c8a", lineHeight: 1 }}>{suggestions.length}</div>
                      <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3, fontWeight: 500 }}>Suggestions</div>
                    </div>
                    ...
                    <div onClick={() => setShowCartOnly(o => !o)}
                      ...
                      <div style={{ fontSize: 22, fontWeight: 800, color: "#16a34a", lineHeight: 1 }}>{inCartSuggestions.length}</div>
                      <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3, fontWeight: 500 }}>In Cart</div>
                      {showCartOnly && <div style={{ fontSize: 9, color: "#16a34a", fontWeight: 700, marginTop: 3, letterSpacing: "0.4px" }}>FILTERED ✕</div>}
                    </div>
                    {/* Checkout / Buy Now */}
                    <div style={{ background: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 12, padding: "12px 14px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontSize: 22, fontWeight: 800, color: "#7c3aed", lineHeight: 1 }}>${cartTotal2.toFixed(0)}</div>
                        <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3, fontWeight: 500 }}>Checkout</div>
                      </div>
                      {inCartSuggestions.length > 0 && (
                        <button onClick={() => setShowCartPanel(true)}
                          style={{ marginTop: 8, background: "#7c3aed", color: "#fff", border: "none", borderRadius: 7, padding: "5px 0", fontSize: 11, fontWeight: 700, cursor: "pointer", width: "100%" }}>
                          Buy Now →
                        </button>
                      )}
                    </div>
                  ...
                  {suggestions.length === 0 ? (
                    ...
                    <div style={{ textAlign: "center", padding: "32px 24px", color: "#9ca3af" }}>
                      ...
                      <button onClick={() => setShowCartOnly(false)} style={{ marginTop: 10, background: "none", border: "1px solid #e2e8f0", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#6b7280", cursor: "pointer" }}>Show all suggestions</button>
                    </div>
                  ) : visibleSuggestions.map(({ part, reasons, priority }) => {
                    const inCart = cart.find(i => i.id === part.id);
                    const price = part.retailPrice || part.price;
                    return (
                      <div key={part.id} style={{ background: "#fff", border: `1px solid ${PRIORITY_CFG[priority].color}30`, borderLeft: `3px solid ${PRIORITY_CFG[priority].color}`, borderRadius: 12, marginBottom: 10, padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: 12 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                            {part.url
                              ? <a href={part.url} target="_blank" rel="noreferrer" style={{ fontSize: 14, fontWeight: 700, color: "#0f4c8a", textDecoration: "none" }}>{part.name} ↗</a>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
                          <span style={{ fontSize: 17, fontWeight: 800, color: "#0f4c8a" }}>${price}</span>
                          {part.vendor !== "custom" && (
                            inCart
                              ? <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                                  <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 700 }}>✓ In Cart</span>
                                  <button onClick={() => removeFromCart(part.id)} style={{ background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}>✕</button>
                                </div>
                              : <button onClick={() => addToCart(part)} style={{ background: "#0f4c8a", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>+ Add</button>
                          )}
                        </div>
                      </div>
                    );
      ...
      {showAddEquip && (
        <div style={s.modalBg}>
          <div style={s.modalBox}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowAddEquip(false)} style={{ flex: 1, padding: 10, border: "1px solid #e2e8f0", borderRadius: 8, background: "#fff", cursor: "pointer", fontWeight: 600 }}>Cancel</button>
              <button onClick={addEquipment} style={{ flex: 1, padding: 10, border: "none", borderRadius: 8, background: "#0f4c8a", color: "#fff", cursor: "pointer", fontWeight: 700 }}>Add</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── CART PANEL ── */}
      {showCartPanel && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 200 }} onClick={() => setShowCartPanel(false)}>
          <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: 340, background: "#fff", boxShadow: "-4px 0 32px rgba(0,0,0,0.14)", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid #e8eaed", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setShowCartPanel(false)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#6b7280" }}>✕</button>
            </div>
            <div style={{ flex: 1, overflowY: "auto" }}>
              {cart.length === 0
                ...
                : cart.map(item => (<div key={item.id} style={{ padding: "14px 20px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{item.name}</div><div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>Qty: {item.qty} · ${(item.retailPrice * item.qty).toFixed(2)}</div></div><button onClick={() => removeFromCart(item.id)} style={{ background: "none", border: "none", color: "#dc2626", cursor: "pointer", fontSize: 16 }}>✕</button></div>))}
            </div>
            {cart.length > 0 && (
              <div style={{ padding: 20, borderTop: "1px solid #e8eaed" }}>
                ...
                <button onClick={() => { setShowCartPanel(false); setShowCheckout(true); }} style={{ width: "100%", background: "#0f4c8a", color: "#fff", border: "none", borderRadius: 10, padding: 13, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Checkout →</button>
